...
export function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!email || !password || !name) { setError(t("auth.fillAll", lang)); return }
    if (password.length < 6) { setError(t("auth.passMin", lang)); return }
    if (password !== confirm) { setError(t("auth.passMatch", lang)); return }
    setLoading(true); setError("")
    try {
      await register(email, password, role)
      setSuccess(true)
      setTimeout(() => router.push("/dashboard"), 1000)
    } catch { setError(t("auth.registerFailed", lang)) }
    finally { setLoading(false) }
  }

  if (success) {
    return (
      <Card className="glass-card border-blue-ocean-500/10 text-center">
        <CardContent className="py-12">
          <div className="text-5xl mb-4">🎉</div>
          <h3 className="text-xl font-bold text-white mb-2">{t("auth.success", lang)}</h3>
          <p className="text-blue-ocean-200/40">{t("auth.successDesc", lang)}</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className="glass-card border-blue-ocean-500/10">
      <CardContent className="pt-6 space-y-4">
        ...
        <div className="space-y-1.5">
          <Label className="text-blue-ocean-200/60 text-xs">{t("auth.name", lang)}</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ivan Petrov"
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            <Label className="text-blue-ocean-200/60 text-xs">{t("auth.confirm", lang)}</Label>
